...
Page({
  ...
  createTrip() { this.setData({ createOpen: true, createForm: { title: '', description: '', start_date: '', end_date: '', currency: 'EUR' }, coverQuery: '', coverResults: [], coverLoading: false, createCoverUrl: '' }) },
  ...
  inputCoverQuery(e) { this.setData({ coverQuery: e.detail.value || '' }) },
  searchCoverImages() {
    const query = String(this.data.coverQuery || this.data.createForm.title || '').trim()
    if (!query) return wx.showToast({ title: '先填写行程名称', icon: 'none' })
    this.setData({ coverLoading: true, coverResults: [] })
    api.searchTripCovers(query).then((data) => {
      const results = Array.isArray(data.photos) ? data.photos : (Array.isArray(data.results) ? data.results : [])
      this.setData({ coverResults: results.slice(0, 6) })
    }).catch((error) => wx.showToast({ title: error.errMsg || '封面搜索失败', icon: 'none' })).finally(() => this.setData({ coverLoading: false }))
  },
  selectCover(e) {
    const item = this.data.coverResults[e.currentTarget.dataset.index]
    const url = item && (item.url || item.full || item.src || item.image)
    if (url) this.setData({ createCoverUrl: url })
  },
  submitCreate() {
    ...
    api.createTrip({ title, description: String(form.description || '').trim() || null, start_date: form.start_date || null, end_date: form.end_date || null, currency: form.currency || 'EUR', day_count: form.start_date && form.end_date ? Math.max(1, Math.round((new Date(form.end_date) - new Date(form.start_date)) / 86400000) + 1) : 1 })
      .then(({ trip }) => {
        const saveCover = this.data.createCoverUrl && trip && trip.id ? api.updateTrip(trip.id, { cover_image: this.data.createCoverUrl }) : Promise.resolve()
        return saveCover.catch(() => {}).then(() => { this.closeCreate(); this.loadTrips(); wx.navigateTo({ url: `../trip/trip?id=${trip.id}` }) })
      })
